...
function newObjectFromArray(arr) {
  const prot = { n: 0 };
  const hasObjects = arr.every((v) => v && typeof v === "object" && !isRObject2(v) && !isComplex(v));
  if (hasObjects) {
    const _arr = arr;
...
function isRVectorAtomic(value) {
  ...
  return isRObject2(value) && atomicRTypes.includes(value.type()) || isRObject2(value) && value.isNa();
}
...
async function newRObject(chan, objType, shelter, ...args) {
  const msg = {
    ...
    data: {
      objType,
      args: replaceInObject(args, isRObject, (obj) => obj._payload),
      shelter
    }
  };
